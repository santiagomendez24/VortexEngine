...
def process_log_entry(shm_map: mmap.mmap, entry_bytes: bytes, header_size: int, channel_id: int) -> int:
    (
        timestamp,
        log_id,
        msg_len,
...
def consume_private_channel(channel_id: int):
    ...
    while True:
        try:
            shm_init = mmap.mmap(-1, 64, tagname=shm_name, access=mmap.ACCESS_READ)
            magic, slab_size, max_slabs, header_size = struct.unpack(HEADER_STATIC_FORMAT, shm_init[:16])
            shm_init.close()

            if magic == MAGIC_EXPECTED:
                print(f"[Python Worker {channel_id}] ✅ Connected | Slabs: {max_slabs} | Slab Size: {slab_size / 1024 / 1024:.2f} MB | Header Size: {header_size} B")
                break
        except Exception:
            pass
    ...
    shm_map = mmap.mmap(-1, total_size, tagname=shm_name, access=mmap.ACCESS_READ)
    ...
    try:
        while True:
            ...
            if local_read < current_write:
                ...
                entry_bytes = shm_map[entry_offset : entry_offset + LOG_ENTRY_SIZE]

                bytes_consumed = process_log_entry(shm_map, entry_bytes, header_size, channel_id)
...
def main():
    ...
    print(f"[Vortex Core] Initializing {NUM_WORKERS} threads")
